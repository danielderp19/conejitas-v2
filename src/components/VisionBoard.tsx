...
export default function VisionBoard() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) { const s = JSON.parse(raw); setElements(s.elements || []); setBg(s.bg || BACKGROUNDS[0].value); if (s.bgImage) setBgImage(s.bgImage); }
    } catch { /* ok */ }
  }, []);

  useEffect(() => {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ elements, bg, bgImage })); } catch { /* ok */ }
  }, [elements, bg, bgImage]);
  ...
  const startDrag = useCallback((e: React.MouseEvent | React.TouchEvent, id: string) => {
    ...
    const onMove = (ev: MouseEvent | TouchEvent) => {
      ...
      setElements(prev => prev.map(x => x.id === dragRef.current!.id ? { ...x, x: Math.max(0, dragRef.current!.elX + dx), y: Math.max(0, dragRef.current!.elY + dy) } : x));
    };
    const onUp = () => {
  ...
  function renderShape(el: BoardEl) {
    const { shape, fill, stroke, w, h } = el;
    if (shape === "circle") return <div style={{ width: "100%", height: "100%", borderRadius: "50%", background: fill, border: stroke && stroke !== "transparent" ? `3px solid ${stroke}` : "none" }}/>;
    if (shape === "heart") return (
      <svg viewBox="0 0 100 90" style={{ width: "100%", height: "100%" }}>
        <path d="M50 85 C50 85 5 55 5 28 C5 14 16 5 28 5 C38 5 46 11 50 18 C54 11 62 5 72 5 C84 5 95 14 95 28 C95 55 50 85 50 85Z" fill={fill} stroke={stroke !== "transparent" ? stroke : "none"} strokeWidth="2"/>
      </svg>
    );
    if (shape === "star") return (
      <svg viewBox="0 0 100 100" style={{ width: "100%", height: "100%" }}>
        <polygon points="50,5 61,35 95,35 68,57 79,91 50,70 21,91 32,57 5,35 39,35" fill={fill} stroke={stroke !== "transparent" ? stroke : "none"} strokeWidth="2"/>
      </svg>
    );
    if (shape === "diamond") return (
      <svg viewBox="0 0 100 100" style={{ width: "100%", height: "100%" }}>
        <polygon points="50,5 95,50 50,95 5,50" fill={fill} stroke={stroke !== "transparent" ? stroke : "none"} strokeWidth="2"/>
      </svg>
    );
    // rect
    return <div style={{ width: "100%", height: "100%", background: fill, border: stroke && stroke !== "transparent" ? `3px solid ${stroke}` : "none", borderRadius: 8 }}/>;
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: "'Poppins',sans-serif" }}>
      ...
      <div style={{ background: "rgba(13,10,26,0.97)", borderBottom: `1px solid ${P.border}`, padding: desktop ? "10px 20px" : "8px 10px", display: "flex", alignItems: "center", gap: desktop ? 8 : 4, flexWrap: "wrap", flexShrink: 0 }}>
        ...
        {/* Agregar imagen */}
        <input ref={fileRef} type="file" accept="image/*" style={{ display: "none" }} onChange={e => { const f = e.target.files?.[0]; if (!f) return; const r = new FileReader(); r.onload = ev => addImage(ev.target?.result as string); r.readAsDataURL(f); e.target.value = ""; }}/>
      ...
      {showBgPicker && (
        <div style={{ background: "rgba(26,15,46,0.98)", border: `1px solid ${P.border}`, borderRadius: 12, padding: 12, margin: "6px 12px 0", animation: "fadeIn 0.2s" }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <input ref={bgFileRef} type="file" accept="image/*" style={{ display: "none" }} onChange={e => { const f = e.target.files?.[0]; if (!f) return; const r = new FileReader(); r.onload = ev => { setBgImage(ev.target?.result as string); setShowBgPicker(false); }; r.readAsDataURL(f); }}/>
            ...
            <span style={{ fontSize: 11, color: P.muted }}>o elige un color:</span>
      ...
      {/* ── Canvas ── */}
      <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: desktop ? "16px 20px 24px" : "10px 10px 20px" }}>
        <div
          ref={boardRef}
                  ...
                  {[[-1,-1],[1,-1],[-1,1],[1,1]].map(([cx,cy],i) => (
                    <div key={i} onMouseDown={e => {
                      ...
                      const onMove = (ev: MouseEvent) => {
                        const dw = (ev.clientX - startX) * cx;
                        const dh = (ev.clientY - startY) * cy;
                        setElements(prev => prev.map(x => x.id === el.id ? { ...x, w: Math.max(40, startW + dw), h: Math.max(30, startH + dh) } : x));
                      };
                      const onUp = () => { window.removeEventListener("mousemove", onMove); window.removeEventListener("mouseup", onUp); };
                      window.addEventListener("mousemove", onMove);
                      window.addEventListener("mouseup", onUp);
                    }} style={{ position: "absolute", [cy === -1 ? "top" : "bottom"]: -5, [cx === -1 ? "left" : "right"]: -5, width: 10, height: 10, borderRadius: "50%", background: P.p1, border: "2px solid #fff", cursor: "nwse-resize", zIndex: 10 }}/>
                  ))}
                </div>
              )}

              {/* Contenido */}
              {el.type === "image" && <img src={el.src} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 4, pointerEvents: "none", display: "block" }} crossOrigin="anonymous"/>}
              {el.type === "shape" && renderShape(el)}
              {el.type === "emoji" && <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: Math.min(el.w, el.h) * 0.7, lineHeight: 1, userSelect: "none" }}>{el.emoji}</div>}
              {(el.type === "text" || el.type === "quote") && (
                editingText === el.id
                  ? <textarea
                      ...
                      style={{ width: "100%", height: "100%", background: "rgba(0,0,0,0.5)", border: "none", outline: "none", resize: "none", color: el.color || "#f0e6ff", fontSize: el.fontSize || 18, fontFamily: el.fontFamily || "Poppins", fontWeight: el.bold ? 700 : 400, fontStyle: el.italic ? "italic" : "normal", textAlign: el.align || "center", padding: 8, borderRadius: 4, cursor: "text" }}/>
                  : <div
...
function ToolBtn({ icon, label, onClick, accent }: { icon: string; label: string; onClick: () => void; accent?: boolean }) {
  ...
  return (
    <button
      ...
      title={label}
      style={{
        background: accent ? (hover ? "linear-gradient(135deg,#9333ea,#db2777)" : "rgba(147,51,234,0.2)") : (hover ? "rgba(255,255,255,0.12)" : "rgba(255,255,255,0.06)"),
        border: `1px solid ${accent ? "rgba(147,51,234,0.5)" : "rgba(168,85,247,0.2)"}`,
