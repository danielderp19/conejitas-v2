  ...
  return (
    ...
    >
      <div
        ...
        style={{
          // Completado: subtarea = verde; tarea principal = rosa suave
          background: isDone ? (isMain ? "rgba(236,72,153,0.13)" : "rgba(52,211,153,0.18)") : isComplete ? "rgba(168,85,247,0.2)" : lvlGrad(node.level),
          borderRadius: 14,
          padding: desktop ? "8px 12px 8px 10px" : "6px 8px 6px 8px",
          display: "flex", alignItems: "center", gap: 4,
          cursor: hasKids ? "pointer" : "default",
          opacity: isDone ? 0.92 : 1,
          border: isDragOver ? "1px solid rgba(168,85,247,0.9)" : isDone ? (isMain ? "1px solid rgba(236,72,153,0.4)" : "1px solid rgba(52,211,153,0.55)") : isComplete ? `1px solid ${P.p1}` : "1px solid rgba(255,255,255,0.1)",
          // Borde izquierdo: prioridad (pendiente) o color de completado (rosa principal / verde subtarea)
          borderLeft: `4px solid ${isDone ? (isMain ? "#ec4899" : "#34d399") : (node.priority ? PRIORITY_COLORS[node.priority] : "transparent")}`,
      ...
      >
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <p style={{ margin:0, fontSize: desktop ? 13 : 12.5, fontWeight:600, color: isDone ? (isMain ? "#f9a8d4" : "#86efac") : "#fff", textDecoration: isDone ? "line-through" : "none", whiteSpace:"normal", wordBreak:"break-word", lineHeight:1.4, transition:"color 0.3s, text-decoration 0.3s" }}>
            {node.title}
          </p>
          {hasKids && (
